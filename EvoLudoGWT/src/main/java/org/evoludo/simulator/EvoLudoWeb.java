...
/**
 *
 * Entry point for EvoLudo labs in browsers or ePubs.
 *
...
public class EvoLudoWeb extends Composite
	 ...
	 */
	private void addView(EvoLudoViews view) {
	 ...
	 */
	protected void changeViewTo(EvoLudoViews newView) {
	 ...
	 */
	@Override
	public void viewActivated(EvoLudoViews aView) {
		activeView = aView;
		update(true);
	}
	...
	protected void setView(String name) {
		...
		EvoLudoViews newView = activeViews.get(name);
		if (newView == null) {
	...
	// /**
	// * Discard changes to command line options in TextArea {@link #evoludoCLO} and
	// * revert to settings of active EvoLudo model.
	// */
	// protected void revertCLO() {
	// evoludoCLO.setText(engine.getProcessedCLO());
	// }
	 ...
	 */
	protected void revertCLO() {
//		evoludoCLO.setText(engine.getRawCLO());
	...
	private final native void addKeyListeners(EvoLudoWeb evoludo) /*-{
		...
		var id = evoludo.@org.evoludo.simulator.EvoLudoWeb::elementID;
		...
		if (!$wnd.EvoLudoUtils.keyListeners.get('keydown-' + id)) {
			$wnd.EvoLudoUtils.keyListeners
					.set(
							...
							function(event) {
								...
								if (evoludo.@org.evoludo.simulator.EvoLudoWeb::keyDownHandler(Ljava/lang/String;)(event.key)) {
									event.preventDefault();
								}
			...
			$wnd.EvoLudoUtils.keyListeners
					.set(
							...
							function(event) {
								// console.log("event "+event.type+", key "+event.key+", code "+event.code);
								if (evoludo.@org.evoludo.simulator.EvoLudoWeb::keyUpHandler(Ljava/lang/String;)(event.key)) {
									event.preventDefault();
								}
								event.stopPropagation();
							});
	 ...
	 *      Key Values</a>
	 * @see EvoLudoViews#keyUpHandler(String) EvoLudoViews.keyUpHandler(String) and
	 ...
	 *      "https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent/key/Key_Values">Mozilla
	 *      Key Values</a>
	 * @see EvoLudoViews#keyDownHandler(String) EvoLudoViews.keyDownHandler(String) and
	 *      implementing classes for further keys that may be handled by the current
	 *      view
	 */
			...
			.then(function(zip) {
				zip.forEach(function (relativePath, zipEntry) {
					zip.file(zipEntry.name).async("string").then(function (data) {
						evoludo.@org.evoludo.simulator.EvoLudoWeb::restoreFromFile(Ljava/lang/String;Ljava/lang/String;)(zipEntry.name, data);
					});
				});
			}, function (e) {
				...
				reader.onload = function(e) {
					evoludo.@org.evoludo.simulator.EvoLudoWeb::restoreFromFile(Ljava/lang/String;Ljava/lang/String;)(file.name, e.target.result);
				}
	...
	/*-{
		$wnd.insertEPubEvoLudoLab = $entry(@org.evoludo.simulator.EvoLudoWeb::insertEPubEvoLudoLab(Lcom/google/gwt/dom/client/Element;Ljava/lang/String;));
	}-*/;

	public static native void exportInsertEvoLudoLab()
	/*-{
		$wnd.insertEvoLudoLab = $entry(@org.evoludo.simulator.EvoLudoWeb::insertEvoLudoLab(Lcom/google/gwt/dom/client/Element;Ljava/lang/String;));
	}-*/;
	...
	/*-{
		$wnd.createEvoLudoLab = $entry(@org.evoludo.simulator.EvoLudoWeb::createEvoLudoLab(Ljava/lang/String;Ljava/lang/String;));
	}-*/;
