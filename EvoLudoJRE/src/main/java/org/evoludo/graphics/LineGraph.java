...
class LineQueue extends LinkedList<Data> {
	...
	public GeneralPath getPath(int idx, FrameLayer frame) {
		if( idx<0 || idx>=root.state.length ) return null;
...
// what is the apparently heuristic -2 in xMap and yMap?
		...
		double xStart = getLast().time-(x.upper-x.lower);	// the time of the most recently added data point corresponds to x.upper
...
// what is the apparently heuristic -1 in iy?
			...
			if( data.connect ) path.lineTo(ix, iy);
			else path.moveTo(ix, iy);
		}
		return path;
	}

	// apparently SVGGraphics2D does not support clipping but we can do it ourselves!
...
// what is the apparently heuristic -2 in xMap and yMap? - see above
		double xMap = (canvas.width-2)/(x.upper-x.lower);
//		double xStart = ((LineQueueData)getLast()).time-(x.upper-x.lower);	// the time of the most recently added data point corresponds to x.upper
		double xStart = getLast().time-(x.upper-x.lower);	// the time of the most recently added data point corresponds to x.upper
		double yMap = (canvas.height-2)/(y.upper-y.lower);
		GeneralPath path = new GeneralPath();
// the term 1.000001 looks like another heuristic hack...
		Rectangle2D.Double clip = new Rectangle2D.Double(xStart, y.lower, (x.upper-x.lower)*1.000001, (y.upper-y.lower)*1.000001);
...
// what is the apparently heuristic -1 in iy? - see above
	...
	private static Point2D.Double intersection(Line2D.Double line, Rectangle2D.Double rect) {
		...
		if( Line2D.linesIntersect(line.x1, line.y1, line.x2, line.y2, rect.x, rect.y+rect.height, rect.x+rect.width, rect.y+rect.height) )
			return intersectLines(line, edge);
		// right edge
		edge.setLine(rect.x+rect.width, rect.y, rect.x+rect.width, rect.y+rect.height);
		if( Line2D.linesIntersect(line.x1, line.y1, line.x2, line.y2, rect.x+rect.width, rect.y, rect.x+rect.width, rect.y+rect.height) )
			return intersectLines(line, edge);
		// bottom edge
		...
		// should not get here...
System.out.println("HELP!!! line should intersect: "+line.intersects(rect)+" line=("+line.getP1()+", "+line.getP2()+"), rect="+rect);
...
// note: amalgamate svgQueue and svgPlot and let AbstractGraph take care of clear() and deactivate()
	@Override
	public void clear() {
		super.clear();
		if( doSVG )	{
	...
	public void plot(Graphics2D g2d) {
		if( state.time<=timestamp ) {
			if( doSVG ) svgQueue.append(state);
			return;	// up to date
		}
...
// note: improve dx - keep track of rounding errors
		...
		// int dx = Math.max(1, (int)((currData[0]-prevData[0])*ixRange*plotdim.width+0.5));
...
if( state.pastState[i]!=state.pastState[i] || state.state[i]!=state.state[i] ) continue;
...
// what is the apparently heuristic -1 in x and width?
	...
	protected void printComponent(Graphics g) {
		...
		for( int i=0; i<nStates; i++ ) {
			...
			// SVGGraphics2D ignores clipping !@#$%^& - we have to do it ourselves (works for PDFGraphics2D)
