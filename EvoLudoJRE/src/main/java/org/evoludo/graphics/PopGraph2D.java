...
public class PopGraph2D extends AbstractGraph implements Network.LayoutListener {
	...
	protected void checkGraph() {
		Model.Type model = controller.getEngine().getModel().getModelType();
		switch( model ) {
			case ODE:
	...
	//note: upon initialization reset is called twice: once population fires reset and once resize! all kinds of opportunities for race conditions!
	@Override
	...
	protected void changeVisibleLinkFraction(double newFraction) {
		...
		if( network.nLinks*oldfLinks>MAX_LINK_COUNT ) {
			...
			if( network.nLinks*fLinks>MAX_LINK_COUNT ) return;
	...
	// allow sublcasses to handle resetting the zoom differently (e.g. PopGraph3D should preserve the view position)
	...
	protected void calcBounds() {
		...
		int dw = 1; int dh = 1; int dR = 1; int dw2 = 0; int dh3 = 0;
		...
		switch( type ) {
			...
			case SQUARE:
				...
				if( isHierarchy ) {
					...
					if( hPeriods==null || hPeriods.length!=hLevels ) hPeriods = new int[hLevels];
...
//		drawLinks(plot);
	...
	protected boolean mouseDrag(Point loc, int mode, int stage) {
		...
		switch( mode ) {
			case MOUSE_GRAB:
				if( frame.msg!=null || model==Model.Type.PDE )
					return false;
				switch( stage ) {
					case MOUSE_DRAG_START:
						if( !canvas.contains(loc) ) return false;	// do not even get started!
			...
			case MOUSE_DRAW:
				int node = -1;
				if( !canvas.contains(loc) || frame.msg!=null || model==Model.Type.PDE )
					return false;
				switch( stage ) {
					case MOUSE_DRAG_START:
						// set reference node but do not start drawing yet
...
//						((PopListener)controller).mouseHitNode(node, refnode, tag);	// population signals change back to us
					...
					case MOUSE_DRAG_END:
						// if refnode has not been set, perform strategy change (this is a click rather than drawing)
						if( !isMouseDrawing ) {
							node = findNodeAt(loc);
							// make sure changes get repainted
							if( hasHistory )
...
//		drawNodes(g, canvas, network, forceRepaint);
 ...
 // note: bounds and frame not properly synchronized
			...
			case HONEYCOMB:
				...
				if( side%2==0 ) hex.translate(0, -dh*side);
...
//		drawLinks(g, canvas, network, geometry);
	...
	private static void drawLinks(Graphics2D g, Rectangle canvas, Path2D links, double radius) {
		...
		// note: drawLine accepts only integers, which is highly suspicious for a scaled Graphics2D
	...
	protected int findNodeAt(int x, int y, Rectangle canvasRect) {
		...
		switch( geometry.getType() ) {
			case TRIANGULAR:
				...
				int rx = y-1-r*dh;
				int cx = x-1-c*dw2;
				if( c%2+r%2 == 1 ) rx = dh-rx;
				double loc = (double)rx/(double)dh+(double)cx/(double)dw2;
				if( (c==0 && loc < 1.0) || (c==side && loc > 1.0) ) return -1;
				if( loc < 1.0 ) c--;
				return r*side+c;
			...
			case HONEYCOMB:
				...
				if( r<0 || c<0 || r==side || c==side ) return FINDNODEAT_OUT_OF_BOUNDS;
				return r*side+c;
			...
			default:
				...
				Node2D[] nodes = network.toArray();
				// in the undesirable case that nodes overlap, nodes with a higher index are drawn later (on top)
