...
public class HistoFrameLayer extends FrameLayer {
	...
	int binwidth;
	HistoData data;
	private final java.util.List<HistoMarkedBin> marks = new ArrayList<>();
	...
	public boolean updateMarkedBin(int idx, double binvalue, Color bincolor) {
		...
		if (bin != b.bin)
			changed = true;
		if (!bincolor.equals(b.color))
			changed = true;
		if (changed)
			marks.set(idx, new HistoMarkedBin(bin, bincolor));
		return changed;
	}
	 ...
	 * int start =
	 * (int)((xaxis.lower-xaxis.min)/(xaxis.max-xaxis.min)*data.bins+0.5);
	 * return start+(coord-canvas.x)/binwidth;
	 * }
	 * 
	 * private int convertX2Coord(double xcoord) {
	 * return
	 * canvas.x+(int)((xcoord-xaxis.min)/(xaxis.max-xaxis.min)*canvas.width+0.5);
	 * }
	 */
