...
public class HistoGraph extends AbstractGraph {
	...
	private static final double[][] autoscale = new double[][] { { 100.0, 40.0, 3.0 }, { 50.0, 20.0, 4.0 },
			{ 25.0, 8.0, 4.0 }, { 10.0, 4.0, 4.0 }, { 5.0, 0.0, 4.0 } };
	...
	public void reset(boolean clear) {
		...
		// // needs to be called only after potential changes to the frame such that new
		// binwidth is known
	...
	public void reinit() {
		...
		// this may get called before bounds are set and hence before the binwidth could
		// be determined - skip if this is the case.
		// controller.verifyMarkedBins((HistoFrameLayer)frame, tag);
		frame.init(getBounds());
	...
	protected void prepare() {
		...
		if (gtip != null && gtip.isShowing())
			gtip.setTipText(getToolTipText(infobin));
	 ...
	 * return
	 * Math.max(Math.min((int)((xcoord-x.min)/(x.max-x.min)*(double)data.bins),
	 * data.bins-1), 0);
	 * }
	 * 
	...
	private String getToolTipText(int bin) {
		...
		if (name != null)
			tt += "<i>" + name + "</i><br>";
		return tt + "<i>" + x.label + ":</i> " + x.formatter.format(xLow + xDelta * 0.5) + " - ["
				+ x.formatter.format(xLow) + ", " + x.formatter.format(xLow + xDelta) + ")<br><i>" + y.label + ":</i> "
				+ x.formatter.format(data.state[bin]) + y.unit;
	}
