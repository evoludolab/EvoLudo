...
public class FrameLayer extends JComponent {
	...
	protected java.util.List<GraphLabel>	tickLabels = new ArrayList<GraphLabel>();
	protected java.util.List<GraphLabel>	axisLabels = new ArrayList<GraphLabel>();
	protected java.util.List<GraphLabel>	annotations = new ArrayList<GraphLabel>();
	private final java.util.List<FrameThreshold>	thresholds = new ArrayList<FrameThreshold>();
	public GraphAxis xaxis = new GraphAxis();
	public GraphAxis yaxis = new GraphAxis();
	public GraphStyle style;
	public java.util.List<GraphLabel> labels = new ArrayList<GraphLabel>();
	...
	public void init() {
		...
		Rectangle myFrame = new Rectangle(canvas.x-adjust, canvas.y-adjust, canvas.width+adjust, canvas.height+adjust);
	...
	protected void initGraphRect(Rectangle bounds) {
		...
		if( yaxis.enabled ) {
			...
			if( yaxis.majorTicks>=0 ) {
				ytickwidth = Math.max(style.tickMetrics.stringWidth(yaxis.formatter.format(yaxis.max/Math.PI)+yaxis.unit), 
			...
			else if( yaxis.minorTicks>0 ) right += style.tickMinorLength;
		}
		if( hasLabels ) {
// note: how to determine the sequence of labels? - this must be specified in a flexible way!
		...
		if( xaxis.enabled ) {
			if( xaxis.showLabel && xaxis.label!=null ) bottom += style.labelMetrics.getHeight()+style.gapAxisLabel;
			if( xaxis.majorTicks>=0 ) bottom += style.tickMajorLength+style.tickMetrics.getHeight()+style.gapTickLabel;
			else if( xaxis.minorTicks>0 ) bottom += style.tickMinorLength;
		}
		if( hasLabels ) {
	...
	protected void formatAxisLabels() {
		...
		if( xaxis.enabled ) {
			if( xaxis.showLabel && xaxis.label!=null ) {
				int ylshift = canvas.y+canvas.height+style.tickMajorLength+style.gapTickLabel+style.tickMetrics.getHeight();
				axisLabels.add(new GraphLabel(xaxis.label, canvas.x+(canvas.width-style.labelMetrics.stringWidth(xaxis.label))/2, 
						ylshift+style.gapAxisLabel+style.labelMetrics.getHeight()-style.labelMetrics.getDescent()));
			}
		}
		if( yaxis.enabled ) {
			if( yaxis.showLabel && yaxis.label!=null ) {
				int xlshift = canvas.x+canvas.width+style.tickMajorLength+style.gapTickLabel;
				int ytickwidth = Math.max(style.tickMetrics.stringWidth(yaxis.formatter.format(yaxis.max/Math.PI)+yaxis.unit), 
										  style.tickMetrics.stringWidth(yaxis.formatter.format(yaxis.min/Math.PI)+yaxis.unit));
				GraphLabel label = new GraphLabel(yaxis.label, xlshift+ytickwidth+style.gapAxisLabel+style.labelMetrics.getHeight()-style.labelMetrics.getDescent(), canvas.y+(canvas.height+style.labelMetrics.getHeight()+style.labelMetrics.stringWidth(yaxis.label))/2);
				label.vertical = true;
		...
		if( nLabels>=4 ) {
			// the labels only need proper positioning - for now we only deal with the first four: lower-left, upper-left, upper-right, lower-right
			int ytop = -style.gapAxisLabel;
			int ybottom = canvas.height+style.gapAxisLabel+style.labelMetrics.getHeight()-style.labelMetrics.getDescent()-1;
			if( xaxis.majorTicks>=0 ) ybottom += style.tickMajorLength+style.gapTickLabel+style.tickMetrics.getHeight();
			else if( xaxis.minorTicks>0 ) ybottom += style.tickMajorLength;
// note: how to determine the sequence of labels? - this must be specified in a flexible way!
...
//		int ylshift = yshift+style.tickMajorLength+style.gapTickLabel+style.tickMetrics.getHeight();
		...
		if( yaxis.enabled ) {
			double dy;
			if( yaxis.logScale )
				dy = Math.log10(canvas.height+style.frameStrokeWidth)/((yaxis.majorTicks+1)*(yaxis.minorTicks+1));
			else
...
//			int xlshift = xshift+style.tickMajorLength+style.gapTickLabel;
	...
	//		 too much effort to get to the root of this for an increasingly outdated JRE frontend.
	protected synchronized void formatTickLabels() {
		tickLabels.clear();
		int yshift = canvas.y+canvas.height;
		int ylshift = yshift+style.tickMajorLength+style.gapTickLabel+style.tickMetrics.getHeight()-style.tickMetrics.getDescent();
		int xshift = canvas.x;
		...
		if( yaxis.enabled && yaxis.majorTicks>=0 ) {
			...
			for( int i=0; i<=yaxis.majorTicks; i++ ) {
				ytick -= ystep;
				if( i==yaxis.majorTicks ) tickLabels.add(new GraphLabel(yaxis.formatter.format(ytick)+yaxis.unit, xlshift, yshift));
				else tickLabels.add(new GraphLabel(yaxis.formatter.format(ytick)+yaxis.unit, xlshift, yshift+voff));
				yshift += yincr;
			}
		}
	}
	...
	public boolean updateYThreshold(int idx, double level, Color color) {
		if( Double.isFinite(level) ) {
			if( thresholds.size()<=idx ) return false;
		...
		if( Math.abs(level-t.level)>1e-8 ) changed = true;
		if( !color.equals(t.color) ) changed = true;
		if( changed ) thresholds.set(idx, new FrameThreshold(level, color));
		return changed;
	}
	...
	protected void paintComponent(Graphics g) {
		...
		// note: the synchronized block is a hack to prevent concurrent modification exceptions
		//		 on tickLabels when restoring state; too much effort to get to the root of this
		...
		synchronized(this) {
			...
			for( GraphLabel t : axisLabels ) {
				g2.setPaint(t.color);
				if( t.vertical ) drawVerticalString(g2, t.label, t.x, t.y);
				else g2.drawString(t.label, t.x, t.y);
			}
			for( GraphLabel t : labels ) {
				g2.setPaint(t.color);
				if( t.vertical ) drawVerticalString(g2, t.label, canvas.x+t.x, canvas.y+t.y);
				else g2.drawString(t.label, canvas.x+t.x, canvas.y+t.y);
			}
...
//		TextLayout vText = new TextLayout(label, style.labelFont, new FontRenderContext(null, true, false));
