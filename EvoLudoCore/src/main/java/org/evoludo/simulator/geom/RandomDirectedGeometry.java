 ...
 * connectivity before adding remaining edges uniformly at random.
 */
public class RandomDirectedGeometry extends AbstractGeometry {
	...
	public void init() {
		if (size <= 0)
			throw new IllegalStateException("size must be set before initializing a directed random graph geometry");
		isRewired = false;
		isUndirected = false;
		isRegular = false;
		alloc();
