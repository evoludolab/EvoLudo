...
public abstract class Network extends AbstractList<Node> implements Iterator<Node> {
	...
	/**
	 * Settings to determine the mode of the layouting process of networks:
	 * <dl>
	 * <dt>{@link #DEFAULT}
	 * <dd>animate layouting process for small(er) networks. Thresholds based on
	 * {@link #MAX_ANIMATE_LAYOUT_VERTICES_DEFAULT} and
	 * {@link #MAX_ANIMATE_LAYOUT_LINKS_DEFAULT}.
	 * <dt>{@link #ON}
	 * <dd>always animate layouting process
	 * <dt>{@link #OFF}
	 * <dd>never animate layouting process
	 * </dl>
	 * 
	 * @author Christoph Hauert
	 */
	public enum Animate {
		/**
		 * Animate layout process of small(er) networks. Thresholds based on
		 * {@link #MAX_ANIMATE_LAYOUT_VERTICES_DEFAULT} and
		 * {@link #MAX_ANIMATE_LAYOUT_LINKS_DEFAULT}.
		 */
		DEFAULT,
		...
		/**
		 * Always animate layout process
		 */
		ON,

		/**
		 * Never animate layout process
		 */
		OFF;
		...
		/**
		 * Checks if layout process should be animated. By default the layout is
		 * animated for smaller geometries (at most
		 * {@value #MAX_ANIMATE_LAYOUT_VERTICES_DEFAULT} nodes and at most
		 * {@value #MAX_ANIMATE_LAYOUT_LINKS_DEFAULT} links). The layout is never
		 * animated for lattices.
		 * 
		 * @param geometry the structure of the population the geometry to layout
		 * @return {@code true} if layout for {@code geometry} is animated
		 */
		public boolean isAnimated(Geometry geometry) {
